...
def to_hat(expr) -> str:
    """Convert a sympy expression to string using ^ instead of **."""
    import re
    s = str(expr)
    s = re.sub(r'\*\*', '^', s)
    # Remove multiplication sign between coefficient and variable: 3*x -> 3x
    s = re.sub(r'(\d)\*([a-zA-Z])', r'\1\2', s)
    return s

def fmt_term(coeff, var_str, exp_val) -> str:
    """Format coeff * var^exp nicely:  2x², -5x¹, 4, x³"""
    try:
...
# ── Extract (coeff, exponent) from simple power terms ────────────────────────
def extract_ce(term, var):
    """Return (coeff, exp) if term is  c * var^n, else None."""
    if not term.free_symbols or var not in term.free_symbols:
        return None
    if term == var:
...
def differentiate_with_trail(expr_str: str, var_str: str, order: int) -> dict:
    ...
    if all_simple:
        ...
        for t in terms:
            ...
            if ce:
                ...
                e_val = float(ce[1])
                n1    = e_val - 1
                raw_parts.append(f"{c_val:g} * {e_val:g}{var_str}{sup(int(e_val))+'⁻¹' if e_val==int(e_val) else sup(e_val-1)}")
                new_c = c_val * e_val
        ...
        # Remove trailing +0 entries, clean up
        non_zero = [p for p in simp_parts if p != "0"]
        d("= " + "  +  ".join(simp_parts))
    ...
    # ── FALLBACK ──────────────────────────────────────────────────────────────
        ...
        s(f"Apply differentiation rules")
        final = simplify(diff(expr, x, order))
        d(f"= {final}")
        s(f"= {final}", "answer")

    # Convert ** to ^ and clean up * between number and variable
    def clean(t):
        t = t.replace("**", "^")
        # Remove * between digit and variable/function:  4*x -> 4x
        t = re.sub(r'(\d)\*([a-zA-Z(])', r'\1\2', t)
        # Remove * between ) and letter:  )*x -> )x
        t = re.sub(r'\)\*([a-zA-Z])', r')\1', t)
        # Replace * between letter/digit and letter with · (e.g. 2x*sin -> 2x·sin)
        t = re.sub(r'([a-zA-Z0-9])\*([a-zA-Z])', r'\1·\2', t)
        # Add · between variable^n and a function: x^2cos -> x^2·cos
        t = re.sub(r'(\^[\d]+)([a-zA-Z])', r'\1·\2', t)
        # Remove * between x and (: x*(  -> x(
        t = re.sub(r'([a-zA-Z])\*\(', r'\1(', t)
        return t
